...
public class SafeDataStorage {
	...
	private final Map<String, Optional<SavedData>> cache = Maps.newHashMap();
	...
	public <T extends SavedData> T get(SavedData.Factory<T> factory, String id) {
		Optional<SavedData> saveddata = this.cache.get(id);
		if (saveddata != null) {
			return (T) saveddata.orElse(null);
		}
		if (!this.cache.containsKey(id)) {
			T loaded = this.readSavedData(factory.deserializer(), factory.type(), id);
			this.cache.put(id, Optional.ofNullable(loaded));
			return loaded;
		}
		return null;
	}
	...
	@Nullable
	private <T extends SavedData> T readSavedData(BiFunction<CompoundTag, HolderLookup.Provider, T> function, @Nullable DataFixTypes fixTypes, String id) {
		try {
	...
	public void save() {
		this.cache.forEach((id, data) -> data.ifPresent(savedData -> savedData.save(this.getDataFile(id), this.registries)));
	}
}